...
class CameraPoses():
    ...
    def __init__(self, data_dir, skip_frames, intrinsic):
        ...
        #Creating a tello instance to use along the code
        self.tello = Tello()
        self.tello.connect()
        self.initialHeight = self.tello.get_distance_tof() * 100.
    ...
    def decomp_essential_mat_old(self, E, q1, q2):
        ...
        ##ESTIMATING THE SCALE USING THE IMU DATA AND TOF SENSOR OF THE DJI TELLO
        tof = self.tello.get_distance_tof()
        print("TOF",tof)
        height = (tof / 100.) - initial_height #In meters!
        acceleration_x = self.tello.get_acceleration_x() /100
        acceleration_y = self.tello.get_acceleration_y()/100
        acceleration_z = self.tello.get_acceleration_z()/100
        print("ACCELERATIONS\n",acceleration_x, acceleration_y, acceleration_z)

        t2 = cv2.getTickCount()
        dt = (t2 - t1) / cv2.getTickFrequency()
            
        velocity = np.array([[acceleration_x * dt], [acceleration_y * dt]]) #[acceleration_z * dt]])

        # Check if the velocity values are smaller than the threshold
        if np.abs(velocity).max() < 0.1:
            relative_scale = 0
        else:
            # Estimate the relative scale based on the height and velocity information
            relative_scale = height / np.linalg.norm(velocity)
...
with open('C:/Users/jpedr/Documents/Python/VisualOdometry/ComputerVision/LiveCameraTrajectory/intrinsicNew.npy', 'rb') as f:
    intrinsic = np.load(f)
...
while(True):
    ...
    new_frame = vo.tello.get_frame_read().frame
    ...
    if q1 is not None:
        if len(q1) > 20 and len(q2) > 20:
            ...
            #Right after geting the scale, talvez de problema se nao satisfazer o if
            t2 = cv2.getTickCount()
    ...
    estimated_camera_pose_x, estimated_camera_pose_y, estimated_camera_pose_z = cur_pose[0, 3], cur_pose[2, 3], cur_pose[1, 3]

    
    
    old_frame = new_frame
    
    #removed ret and processframe comparation
    ...
    cv2.putText(new_frame, str((cur_pose[0, 3],2)), (540,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1)
    cv2.putText(new_frame, str((cur_pose[1, 3],2)), (540,90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1)
    cv2.putText(new_frame, str((cur_pose[2, 3],2)), (540,130), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1)
    ...
    cv2.imshow("img", new_frame)
        ...
        #vo.tello.takeoff()
    #else:
        #vo.tello.send_rc_control(0,5,0,0)
    frame_counter += 1
...
plt.plot(estimated_path[:,0], estimated_path[:,2], estimated_path[:,1])
#plt.show()
ax.set_xlabel('X axis')
ax.set_ylabel('Y axis')
ax.set_zlabel('Z axis')
